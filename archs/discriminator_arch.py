...
@ARCH_REGISTRY.register()
class VGGStyleDiscriminatorSN(nn.Module):
    """
    使用谱归一化（SN）的VGG风格判别器，输入大小为128 x 128或256 x 256。
    用于训练SRGAN、ESRGAN和VideoGAN。
    ...
    def __init__(self, num_in_ch, num_feat, input_size=128):
        super(VGGStyleDiscriminatorSN, self).__init__()
        self.input_size = input_size
        # 确保输入尺寸为 128 或 256
        assert self.input_size == 128 or self.input_size == 256, (
            f'input size must be 128 or 256, but received {input_size}')
    ...
    def forward(self, x):
        ...
        # 第三层卷积
        feat = self.lrelu(self.bn2_0(self.conv2_0(feat)))  # 输入大小：(B, num_feat*2, H/4, W/4)，输出大小：(B, num_feat*4, H/4, W/4)
        feat = self.lrelu(self.bn2_1(self.conv2_1(feat)))  # 输出空间尺寸减半，输出大小：(B, num_feat*4, H/8, W/8)

        # 第四层卷积
        feat = self.lrelu(self.bn3_0(self.conv3_0(feat)))  # 输入大小：(B, num_feat*4, H/8, W/8)，输出大小：(B, num_feat*8, H/8, W/8)
        feat = self.lrelu(self.bn3_1(self.conv3_1(feat)))  # 输出空间尺寸减半，输出大小：(B, num_feat*8, H/16, W/16)

        # 第五层卷积
        feat = self.lrelu(self.bn4_0(self.conv4_0(feat)))  # 输入大小：(B, num_feat*8, H/16, W/16)，输出大小：(B, num_feat*8, H/16, W/16)
        feat = self.lrelu(self.bn4_1(self.conv4_1(feat)))  # 输出空间尺寸减半，输出大小：(B, num_feat*8, H/32, W/32)

        # 如果输入尺寸为 256，则进行额外的卷积
        if self.input_size == 256:
            feat = self.lrelu(self.bn5_0(self.conv5_0(feat)))  # 输入大小：(B, num_feat*8, H/32, W/32)，输出大小：(B, num_feat*8, H/32, W/32)
            feat = self.lrelu(self.bn5_1(self.conv5_1(feat)))  # 输出空间尺寸减半，输出大小：(B, num_feat*8, H/64, W/64)
...
@ARCH_REGISTRY.register(suffix='srworkflow')
